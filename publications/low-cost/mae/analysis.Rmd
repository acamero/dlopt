...
fit.obs <- function(data, seed, print=FALSE) {
  ...
  output <- data.frame("intercept"=sm$coefficients[1,1], "residual.std.error"=sm$sigma, "perc.error"=sm$sigma/sm$coefficients[1,1], "f.stat"=sm$fstatistic[1], "cor.estimate"=cor.res$estimate, "cor.p.value"=cor.res$p.value, "one.diff.perc"=one.diff)
  if(print) {
...
lms.parking <- lapply(1:30, function(s){ fit.obs(parking, seed=s) })
lms.parking <- do.call(rbind.data.frame, lms.parking)
colMeans(lms.parking)

a <- fit.obs(parking, seed=10, print=TRUE)
